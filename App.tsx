...
const MOCK_OFFERS = [
  { id: 'offer-1', storeId: '2', name: "Burger & Brew", title: "Free Craft Beer", description: "With any signature burger purchase", badge: "Free Drink", image: "https://picsum.photos/seed/beer/300/150", type: 'gift', distance: '0.3 mi' },
  { id: 'offer-2', storeId: '1', name: "The Golden Spoon", title: "20% Off Lunch", description: "Valid Mon-Fri 11am-3pm", badge: "20% OFF", image: "https://picsum.photos/seed/lunch/300/150", type: 'discount', distance: '0.1 mi' },
  { id: 'offer-3', storeId: '3', name: "Sushi Zen", title: "BOGO Special Rolls", description: "Buy one get one free on all special rolls", badge: "1 + 1", image: "https://picsum.photos/seed/rolls/300/150", type: 'deal', distance: '0.4 mi' },
];
                ...
                <div className="fixed inset-0 z-50 flex items-center justify-center px-6">
                    ...
                    <div className="relative bg-white w-full max-w-sm rounded-[2.5rem] p-6 shadow-2xl animate-in fade-in zoom-in duration-200">
                        ...
                        <div className="flex justify-center gap-3 mb-10">
                            {[1, 2, 3, 4, 5].map((star) => (
                                <button key={star} onClick={() => setUserRating(star)} className="p-1 focus:outline-none transition-transform active:scale-95 group">
                                    <Star className={`w-10 h-10 transition-colors duration-200 ${star <= userRating ? 'fill-orange-500 text-orange-500' : 'text-gray-200 fill-gray-50'}`} strokeWidth={star <= userRating ? 0 : 1.5} />
                                </button>
                            ))}
                        </div>
                        <div className="space-y-3">
                            <button onClick={handleSubmitRating} disabled={userRating === 0} className={`w-full py-4 rounded-2xl font-bold text-lg transition-all shadow-lg ${userRating > 0 ? 'bg-orange-600 text-white shadow-orange-200 active:scale-[0.98]' : 'bg-gray-100 text-gray-400 shadow-none cursor-not-allowed'}`}>
                                Submit Review
                            </button>
                            <button onClick={() => setShowRatingModal(false)} className="w-full py-3 rounded-2xl font-bold text-gray-400 hover:text-gray-600 active:scale-[0.98] text-sm">
                                Maybe Later
                            </button>
                        </div>
                    </div>
      ...
      return (
          <div className="h-full w-full bg-white flex flex-col animate-in slide-in-from-right duration-300 relative">
              <div className="h-64 w-full bg-gray-200 relative shrink-0">
                  <img src={`https://picsum.photos/seed/${selectedStoreId || 'hero'}/800/600`} alt="Restaurant Hero" className="w-full h-full object-cover" />
                  <div className="absolute inset-0 bg-gradient-to-t from-black/80 via-black/20 to-transparent"></div>
                  <button onClick={goBack} className="absolute top-0 left-0 m-4 mt-safe-top p-3 bg-white/10 backdrop-blur-md border border-white/20 rounded-full text-white hover:bg-white/20 transition-colors active:scale-95">
              ...
              <div className="flex-1 bg-white relative -mt-6 rounded-t-[2rem] overflow-hidden flex flex-col shadow-[0_-5px_20px_rgba(0,0,0,0.15)] isolate">
                  <div className="pt-6 pb-2 pl-6 overflow-x-auto no-scrollbar">
                      <div className="flex items-center gap-3 pr-6">
                        {CATEGORIES.map((cat) => (
                            <button key={cat} onClick={() => setSelectedCategory(cat)} className={`px-6 py-2.5 rounded-full text-base font-bold whitespace-nowrap transition-all duration-300 border ${selectedCategory === cat ? 'bg-gray-900 text-white border-gray-900 shadow-lg scale-[1.02]' : 'bg-gray-50 text-gray-500 border-transparent hover:bg-gray-100'}`}>
                                {cat}
                            </button>
                        ))}
                      </div>
                  ...
                  <div className="flex-1 overflow-y-auto px-6 py-4 space-y-6 no-scrollbar pb-32">
                      ...
                      {filteredItems.length === 0 ? (
                          <div className="py-10 text-center text-gray-400"><p>No items in this category yet.</p></div>
                      ) : (
                          filteredItems.map((item) => (
                              <div key={item.id} className="flex gap-4 group">
                                  <div className="flex-1 py-1">
                                      <h3 className="font-bold text-gray-900 text-[17px] leading-tight">{item.name}</h3>
                                      <p className="text-gray-500 text-sm mt-1.5 leading-relaxed line-clamp-2">{item.description}</p>
                                      <div className="mt-3 flex items-center gap-3"><span className="font-bold text-gray-900 text-base">{item.price}</span></div>
                                  </div>
                                  <div className="w-28 h-28 bg-gray-100 rounded-2xl overflow-hidden relative shrink-0 shadow-sm">
                                      <img src={item.image} className="w-full h-full object-cover" alt={item.name} />
                                      <button onClick={() => handleAddToCart(item)} className="absolute bottom-2 right-2 w-9 h-9 bg-white rounded-full shadow-lg flex items-center justify-center text-orange-600 active:scale-90 transition-transform hover:bg-gray-50 z-10">
              ...
              <div className="absolute bottom-8 left-1/2 -translate-x-1/2 z-20 w-full px-6 max-w-[440px]">
                  <button onClick={() => setCurrentActivity(Activity.CART)} className="w-full bg-gray-900 text-white p-4 rounded-full shadow-2xl flex items-center justify-between active:scale-[0.98] transition-transform">
                      <div className="flex items-center gap-3">
                        <div className="bg-white/20 p-2 rounded-full relative">
                            <ShoppingBag className="w-5 h-5" />
                            {cartItemCount > 0 && <div className="absolute -top-1 -right-1 w-4 h-4 bg-orange-500 rounded-full border-2 border-gray-900 text-[9px] flex items-center justify-center font-bold">{cartItemCount}</div>}
                        </div>
  ...
  if (currentActivity === Activity.FULL_SCANNER) {
      return (
          <div className="h-full w-full bg-black relative animate-in fade-in duration-300">
              <QRScanner onScan={handleScan} isScanning={true} isFullScreen={true} onClose={() => setCurrentActivity(Activity.HOME)} />
          </div>
      );
  }

  // --- HOME ACTIVITY (REDESIGNED) ---
  return (
    <div className="relative h-full w-full flex flex-col bg-gray-50 overflow-hidden">
      
      {/* Search Bar - Fixed Top, Premium Style */}
      <div className="sticky top-0 left-0 right-0 z-40 bg-gray-50/80 backdrop-blur-lg border-b border-gray-200/50 px-5 pt-safe-top pb-3 flex items-center gap-2">
         <div className="mt-4 flex-1 bg-white shadow-sm rounded-full flex items-center px-4 py-2.5 border border-gray-200 transition-all active:ring-2 active:ring-orange-100">
            <Search className="w-5 h-5 text-orange-500 shrink-0" />
            <input 
                type="text" 
                placeholder="Find a restaurant..." 
                className="bg-transparent border-none outline-none text-gray-900 placeholder-gray-400 flex-1 px-3 text-sm font-medium"
            />
            <button className="p-1 hover:bg-gray-50 rounded-full transition-colors text-gray-400">
      ...
      <div className="flex-1 overflow-y-auto no-scrollbar pb-20">
        ...
        {/* Scan & Dine Hero - Primary Action */}
        <section className="px-5 py-8 flex flex-col items-center animate-in fade-in slide-in-from-top-4 duration-700">
           <div className="w-full bg-zinc-950 rounded-[2.5rem] p-8 relative overflow-hidden flex flex-col items-center text-center shadow-xl">
              {/* Background Design */}
              ...
              <div className="relative mb-5">
                 <div className="w-20 h-20 bg-zinc-900 border border-white/10 rounded-3xl flex items-center justify-center shadow-2xl">
                    <QrCode className="w-10 h-10 text-orange-500" />
                 </div>
              </div>

              <h2 className="text-2xl font-black text-white tracking-tight mb-2">Scan & Dine</h2>
              <p className="text-zinc-500 text-sm font-medium leading-relaxed max-w-[200px] mb-8">
                 Scan a table QR code to view the menu
              </p>

              <button 
                 onClick={() => setCurrentActivity(Activity.FULL_SCANNER)}
                 className="w-full max-w-[240px] bg-gradient-to-r from-orange-500 to-orange-600 text-white py-4 rounded-full font-bold text-base shadow-lg shadow-orange-900/20 active:scale-95 transition-all flex items-center justify-center gap-3 border border-white/10"
              >
        ...
        <section className="pb-8 animate-in fade-in slide-in-from-right duration-500 delay-200">
           <div className="flex items-center justify-between px-6 mb-4">
              <h2 className="text-xl font-bold text-gray-900 tracking-tight">Special Offers</h2>
              <span className="text-[10px] font-bold text-gray-400 uppercase tracking-widest bg-gray-100 px-2 py-1 rounded-md border border-gray-200">Sponsored</span>
           </div>
           
           <div className="flex gap-5 overflow-x-auto no-scrollbar pb-4 px-5 snap-x">
              {MOCK_OFFERS.map(offer => (
                  <button 
                      key={offer.id} 
                      onClick={() => handleStoreSelect(offer.storeId)} 
                      className="snap-start shrink-0 w-72 bg-white rounded-[1.5rem] shadow-[0_4px_20px_-8px_rgba(0,0,0,0.1)] border border-gray-100 overflow-hidden group active:scale-[0.98] transition-all relative flex flex-col"
                  >
                     <div className="h-40 bg-gray-100 relative overflow-hidden shrink-0">
                        ...
                        <div className="absolute top-3 left-3 flex flex-col items-start gap-2">
                             <div className={`inline-flex items-center gap-1.5 px-3 py-1.5 rounded-xl text-[11px] font-bold shadow-sm backdrop-blur-md border border-white/10 text-white ${offer.type === 'discount' ? 'bg-orange-500/90' : 'bg-emerald-500/90'}`}>
                                {offer.type === 'discount' ? <Percent className="w-3 h-3" /> : <Gift className="w-3 h-3" />}
                                {offer.badge}
                             </div>
                        </div>

                        <div className="absolute bottom-3 left-3">
                            <span className="text-[10px] font-bold text-white bg-black/40 backdrop-blur-md px-2.5 py-1 rounded-full border border-white/10 flex items-center gap-1">
                     ...
                     <div className="p-4 flex flex-col gap-1 items-start w-full">
                        <h3 className="font-extrabold text-gray-900 text-[17px] leading-tight w-full truncate text-left">{offer.title}</h3>
                        <div className="flex items-center gap-2 text-gray-500 w-full mt-0.5">
                           <span className="font-bold text-xs truncate text-gray-400">{offer.name}</span>
                           <span className="w-1 h-1 bg-gray-200 rounded-full shrink-0"></span>
                           <span className="text-[9px] bg-gray-50 px-1.5 py-0.5 rounded text-gray-400 font-bold uppercase tracking-wider border border-gray-100">Ad</span>
                        </div>
                        <p className="text-gray-400 text-xs mt-2 line-clamp-1 text-left font-medium leading-relaxed w-full">
                            {offer.description}
                        </p>
                     </div>
                  </button>
        ...
        {/* Best Nearby Restaurants List */}
        <div className="flex-1 bg-white rounded-t-[2.5rem] shadow-[0_-8px_30px_rgba(0,0,0,0.03)] pt-4">
           <NearbyList 
